  ...
  <div class="comment-list">
    ...
    <div v-for="(item, i) in list" :key="item._id" class="item" >
      <div class="item-header">
        ...
        <div class="info">
          <div class="name"> {{item.user.name}} {{item.user.type === 0 ? '(作者)' : ''}} </div>
      ...
      <div v-for="e in item.other_comments" :key="e._id" class="item-other" >
        <div class="item-header">
          ...
          <div class="info">
            <div class="name"> {{e.user.name}} {{e.user.type === 0 ? '(作者)' : ''}} </div>
            <div class="time"> {{formatTime(e.create_time)}} </div>
          </div>
        </div>
        <div class="comment-detail"> {{'@' + e.to_user.name}} {{e.to_user.type === 0 ? '(作者)' : ''}}：{{e.content}}</div>
        <div class="item-comment">
          <div class="message">
    ...
    <Comment :visible="state.visible" :to_user="state.to_user" :comment_id="state.comment_id" :article_id="article_id" @ok="handleOk" @cancel="handleCancel"/>
  </div>
</template>
<script lang="ts">
